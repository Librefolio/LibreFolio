...
- **PAC allocation analysis** can now copy a privacy-safe Asset catalogue and OWNER broker cash snapshot, combine copied or manual funding, keep source facts locked while targets remain editable, model arbitrary quote bases and separate quantity/monetary steps, and explain explicit FX valuation. The current P1 report remains analysis-only: it places no orders, routes no cash and claims no solver optimum.
