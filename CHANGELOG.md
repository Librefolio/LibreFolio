...
- **PAC allocation analysis (P1)** compares current holdings with explicit target weights using exact decimal arithmetic. The responsive editor can copy every OWNER custody context for a selected portfolio asset, retain assets without saved prices, accept fully manual rows, model cash and contributions per currency, and report missing or invalid inputs without creating trades. Copied facts remain editable snapshots: date or source changes mark them stale, and refresh or removal cannot silently discard edits.
