...
| Aggregazione input | Target globale per Asset canonico; custodie/route Asset×Broker separate; liquidita distinta per sorgente/Broker/valuta e mai convertita automaticamente in una sola cassa. |
...
vendite opzionali e limiti per titolo; liquidita distinta per
sorgente/Broker/valuta; lista nuovi contributi; route/trasferimenti e FX
single-hop espliciti; fee, riserve e margini. Quantità titoli/importo cash sono
output backend.

**Raccordo storico D, 2026-09-08, superato in parte:** le righe
Asset×Broker restano per custodia/route e chiave opaca, ma il target finale è
aggregato per identità Asset canonica. Il core non interpreta ruoli/DB.
L'input manuale resta possibile senza Asset ID DB.

**Casse separate:** disponibilita EUR e USD non sono una cassa unica e neppure
sono aggregate fra Broker. Acquisti/vendite alimentano il ledger
Broker/valuta pertinente; contributi nuovi non entrano due volte nello
snapshot. Con FX disabilitato, una valuta in eccesso non finanzia
automaticamente un'altra. Con FX abilitato, mostrare conversione proposta,
importi debitati/accreditati, tasso, costi/margini e cassa finale.
...
implementato. Round 7 pianifica input manuali/prefilled per-run, funding e Broker
separati, snapshot review immutabile, output Broker-facing e stale guards.
Nessuna migration planner v1. **Taglia:** M per snapshot autorizzati + L per
editor/report/grafici.
...
### Aggiornamento 2026-09-18 — solo le voci che si sono mosse

| Sprint | Variazione dal 2026-09-11 |
|---|---|
| SP06 | 🟡 G1a/G1b/G1c chiusi e committati su I (`8ed7a0f0d`): P&L per broker, candele sintetiche, storico proventi con segno. Corretti 3 bug `abs()` preesistenti sul segno nella contabilita a 3 pool. Batch 2 (aggregati costi/depositi/acquisti, split nuovo-vs-reinvestito, selettori finestra) in checkpoint. Nessuna integrazione in target: I resta sul proprio ramo fino alla review developer della UI. |
| SP11 | 🟡 J completo e committato (`580bd504f`), gia confluito nel ramo D. Non in target: rientra con D. |
| SP13–SP14 | 🟢 **Fetta autorizzata chiusa: `operation="plan"` produce un risultato reale.** Redesign FX radicale chiuso (`0088748a8`). Step3 completo in cinque stage: Stage 1 `oracle.py` (verita di riferimento esaustiva), Stage 2 `constraints/objectives/compiler`, Stage 3 `solver.py` con il fix HALF_UP e il gate oracolo permanente, Stage 4 `proof.py` (promozione floating→provato resa **irrappresentabile**), Stage 5a `wire_numbers.py`/`planner_report.py` (proiezione pura) e Stage 5b `planner.py` (`plan_pac_allocation`). Percorso end-to-end verificato da una richiesta wire reale: `ready_no_op` e `ready_incumbent` entrambi `optimal_proven`, rivalidati contro l'intera unione `PAC_PLAN_OUTPUT_ADAPTER`. 877 test verdi, zero regressioni. **Niente in target**: tutto sul ramo di D, e Stage 3 attende ancora il commit del developer. Fuori perimetro e non iniziati: SELL, policy del rebalancer, registrazione Tool `operation="plan"`, frontend. Gate G-PAC resta aperto sul solo campionamento a scala. |
...
| Quattro cataloghi i18n | Tutte le UI | Un writer tramite CLI i18n; gli owner forniscono gruppi di chiavi, non riscrivono contemporaneamente i JSON. **Aggiungere le chiavi non e cablarle**: chiavi presenti e mai consumate passano indenni sotto svelte-check, Vitest e build di produzione, e spediscono inglese a IT/FR/ES. `grep TODO(coordinator i18n batch)` fa parte della definition of done su entrambi i lati della cucitura. |
...
### 11.4 Debiti scoperti in esecuzione, di proprieta del coordinatore

Difetti trovati mentre si implementava altro, **deliberatamente non corretti sul momento** perche fuori dallo scope del workstream che li ha visti o perche toccano un file condiviso con una corsia concorrente. Non sono bug del task che li ha scoperti: hanno perimetro proprio e vanno sequenziati come pezzi a se.

| # | Debito | Scoperto da | Perimetro reale | Perche non e stato chiuso subito |
|---|---|---|---|---|
| DBT-1 | `missing_fx_pairs` non viene mai popolato dalle conversioni di importo transazione, ma solo dai percorsi di valutazione. Un BUY in valuta priva di cambio quel giorno viene scartato in silenzio, senza segnale di qualita del dato. | I, batch 2 income (2026-09-18) | **Non e un bug dei grafici.** E un gap di contratto dell'engine: degrada allo stesso modo la contabilita 3-pool e la cash-decomposition, e `PnlCandleSeries` e uscito con la stessa omissione. | Chiuderlo significa dare all'engine un canale di uscita per i fallimenti FX a livello transazione, cioe un cambio di contratto su `portfolio_engine.py` — il file condiviso con l'analisi Risk management. Comportamento attuale documentato nel docstring dello schema e fissato da `test_buy_with_an_unconvertible_currency_is_skipped_not_zeroed`. |
| DBT-2 | `PortfolioHistoryPoint` e un record **misto**: campi in prevalenza livello piu un campo flusso (`external_cash_flow`), affettato in blocco con semantica livello. | I, passata livello/flusso (2026-09-18) | Latente e oggi innocuo: `GrowthChart` non legge mai quel campo (grep = 0 occorrenze). Diventa reale nel momento in cui qualcuno rendesse l'ECF giornaliero come barra accanto ai flussi di proventi: erediterebbe in silenzio il confine sbagliato. | Il tipo non e classificabile come unita, quindi la correzione onesta non e spostare un confine ma separare i due regimi nel record — modifica di contratto su un tipo condiviso, sproporzionata rispetto a un difetto che oggi nessuno puo innescare. Va chiusa quando e se quel campo viene reso. |
| DBT-3 | La tabella esposizioni del planner PAC **deve chiudere a 1** — il contratto wire lo impone (`_validate_weight_availability`, `pac_allocator.py:1264-1267`, e il vincolo gemello sui target a `:2460`) — quindi il residuo non categorizzato va pubblicato come **categoria esplicita propria**, non come fetta mancante. | D, Stage 5a (2026-09-18, corretto in giornata) | **Rettifica a verbale: la prima risposta del coordinator era inemettibile in entrambe le forme proposte.** Marcare il peso "non disponibile" sulla copertura parziale viola `:1264` (la forma non-disponibile e legale solo a denominatore zero, e allora per *tutte* le righe della dimensione con motivo `zero_final_invested`); lasciare la dimensione sommare a meno di 1 viola `:1266`. La lettura corretta e piu forte di entrambe: **quando un contratto wire ha un validator, il validator e la specifica e l'unione di tipi e solo il suo alfabeto.** Il vocabolario dei motivi diceva il vero — `zero_*` sono guardie di divisione per zero, nessun motivo esprime "copertura incompleta" — ma da solo non poteva rivelare che anche il ramo a numero pieno e vincolato. | Chiusura per identita, mai per normalizzazione: peso categoria = somma su asset di `w_asset x peso_esposizione`, residuo = somma di `w_asset x (1 - copertura dichiarata)`, e i due sommano a 1 per costruzione. Il rischio si sposta dai dati alla **etichettatura**: chi costruisce la UI esposizioni PAC deve rendere la fetta residua riconoscibile come "non categorizzato" e non fonderla in nessuna categoria reale, con localizzazione a carico del frontend (il `category_id` sentinella e strutturale, non una stringa da confrontare). Convenzione fissata da due test: due asset, uno Tech e uno non dichiarato, che chiudono a `0.5 + 0.5 = 1` esatto; e la variante a dichiarazione **parziale** (asset al 60% Tech e nient'altro), perche `normalize.py:1093-1109` non impone a nessun asset di chiudere la propria dimensione. |
| DBT-4 ~~costo~~ **RITIRATO — era una conclusione sbagliata, tenuta a verbale** | Registrato il 2026-09-18 come "ogni richiesta `plan` risolta dall'oracolo paga comunque una corsa SCIP, forzata dal contratto perche `ReportedFloatingSolverEvidence.stages` e `min_length=1`". **Falso.** `PlannerSolverEvidence` e un'unione — `SolverNotRunEvidence \| ReportedFloatingSolverEvidence` (`:1519-1522`) — e `_validate_stop_evidence` (`:2683-2688`) pretende evidenza floating **solo** quando `stop_reason != "completed"`, applicando la regola finished/unfinished solo dentro il ramo floating. Quindi `completed` + `not_run` e pienamente legale, e `SolverNotRunReasonCode` (`:700`) ha un unico valore, `allocation.solver_not_required`: nessuno scrive un enum a valore singolo per un percorso che non si aspetta venga preso. La corsa ridondante era una **scelta di implementazione sbagliata**, non un costo imposto. | D, Stage 5b (2026-09-18) | **Perche resta scritto invece di essere cancellato.** La conclusione sbagliata nasce dall'aver verificato un vincolo di campo e essersi fermati li, senza leggere il validator che governa l'unione — cioe dalla violazione esatta di **R1**, scritta dal coordinatore alle 18:12 e infranta dal coordinatore stesso alle 18:39, nello stesso file, in meno di un'ora. Entrambi gli agenti hanno commesso lo stesso errore nello stesso giorno, entrambi conoscendo la regola. Questo dice che classe di regola e R1: non una lacuna di conoscenza che si chiude una volta, ma un cedimento di attenzione ricorrente che **sopravvive al sapere di esistere**. Un registro dei debiti che cancella i propri errori e un registro non verificabile. | Corretto in Stage 5b saltando il solver quando l'oracolo risolve ed emettendo `SolverNotRunEvidence`. Il fix chiude anche il difetto A (`ready_infeasible` irraggiungibile quando il solver gira, perche uno stage infeasible e necessariamente `unfinished` e `completed` lo vieta). Costo reale del fix, da non leggere come indebolimento: la produzione perde un **controllo incrociato diagnostico** — un risultato deciso dall'oracolo non ha piu un incumbent del solver che possa dissentire — ma non una garanzia di correttezza, perche l'oracolo e esaustivo. Quel controllo continua a vivere nel gate permanente di test, che e il luogo dove ha scoperto il difetto HALF_UP. |
| DBT-5 | Sul percorso **infeasible sopra il cap** (oracolo escluso perche il dominio e troppo grande, solver attivo) il risultato degrada onestamente a `ready_no_incumbent`/`not_proven`, ma `stop_reason` vale **`time_limit`** senza che alcun limite di tempo sia scattato: la risoluzione era infeasible. | D, Stage 5b (2026-09-18) | **Limite del contratto congelato, non un difetto nostro.** L'enum ammette solo `completed \| time_limit \| node_limit`; `completed` e vietato perche gli stage sono `unfinished` (uno stage infeasible non ha primale/duale/gap finiti), e nessuno dei due limiti residui e vero. Non esiste una scelta onesta: manca un valore tipo `infeasible` o `no_incumbent`. | Non chiudibile senza toccare lo schema wire congelato, e non e lease di D. Mitigazione gia applicata da test-author, ed e la scelta giusta: il test asserisce l'**invariante robusta** (`stop_reason != "completed"` in presenza di uno stage unfinished) invece di fissare il valore inaccurato — cosi il giorno in cui il contratto guadagna un valore veritiero il test non va aggiornato, perche non aveva mai promesso quello sbagliato. Chi rendera la UI del planner non deve mostrare "limite di tempo" come causa all'utente su questo percorso. |
| DBT-6 | `ChartResolution` e un tipo a **tre valori** (`daily \| weekly \| monthly`, `timeSeriesAggregation.ts:15`) consumato da **12 file**: price chart, candlestick, line chart, `priceChartHelpers`, lotti WAC e confronto lotti, `AllocationHistoryChart`, `chartCoreHelpers.test.ts`, `ResolutionBadge`, il modulo di aggregazione e `GrowthChart`. | Coordinatore, review UI grafici (2026-09-18) | La scala delle larghezze candela chiesta dall'utente ha **otto** pioli (1gg, 1w, 2w, 1m, 3m, 6m, 1y, 2y). Allargare il tipo condiviso renderebbe **silenziosamente incompleta** ogni mappa esaustiva costruita su di esso: `ResolutionBadge` tiene `LABEL_KEY: Record<'weekly' \| 'monthly', string>` e non rende nulla per `daily` di proposito — con cinque pioli in piu quella lookup restituisce `undefined`, senza errori di tipo utili. Il rischio non e nel file di chi cambia, ma nei file di chi non sta guardando. | **Decisione vincolante**: il tipo condiviso resta a tre. La scala vive **locale** a `GrowthChart`, e si mappa su `ChartResolution` solo dove si chiamano davvero gli helper condivisi. I pioli senza equivalente condiviso sono gestiti dalle funzioni di bucketing gia locali al componente. Vietato sia allargare il tipo sia forkare una quasi-copia del modulo. Vale per chiunque tocchi i grafici dopo di noi, non solo per il workstream I. **Aggiornamento 2026-09-18, sera:** la seconda meta di questa risoluzione — "la scala vive locale a `GrowthChart`, i pioli senza equivalente condiviso sono gestiti dalle funzioni di bucketing gia locali al componente" — era **sbagliata di fatto** e va considerata superata. Funzioni di bucketing locali non esistono: i tre aggregatori di `GrowthChart` delegano agli helper condivisi tipizzati su `ChartResolution`, e cinque pioli su otto (`2w, 3m, 6m, 1y, 2y`) non hanno alcun equivalente condiviso a cui mappare. Restare "locali" avrebbe quindi significato riscrivere le semantiche di riduzione nel componente — cioe esattamente la quasi-copia che questa stessa voce vieta. Decisione effettiva (opzione **B**): si generalizza il **parametro** dell'helper privato `groupPointsByBucket` da `ChartResolution` a una funzione-chiave di bucket, con un adattatore che preserva le firme pubbliche. Il tipo condiviso resta a tre valori, `mapDateToBucket` resta intatto, i 12 consumatori non vedono alcun cambiamento. Il divieto originale regge; cambia il modo di rispettarlo. |
| DBT-7 | Nella submode candele del P&L il corpo della candela e quasi sempre **invisibile** su finestre ampie: sui dati di review 310 candele su 354 sono piatte (`high == low`), la mediana dell'escursione e `0.0000`, la massima e `249.69`. **Le cifre percentuali registrate qui in un primo momento erano sbagliate nel denominatore** e vanno lette come corrette di seguito: avevo rapportato la candela piu grande alla campata delle sole **chiusure** (`4391.27`), ottenendo 5,7%, mentre la campata realmente **disegnata** e piu larga — comprende i massimi e minimi delle candele *e* gli estremi delle sovrapposizioni per broker, che in submode candele sono renderizzati e quindi stirano l'asse, piu l'8% di margine aggiunto dalla funzione `yAxis.min`. Il difetto e percio **peggiore** di quanto avevo verbalizzato, non migliore. E un terzo tipo di errore rispetto a R9/R10 (oggetto sbagliato) e a R11 (costrutto sbagliato): qui il costrutto — il rapporto — era **giusto**, ed era sbagliato il **riferimento**, perche avevo preso la campata piu vicina a portata di mano invece di quella effettivamente resa. | I, re-review UI (2026-09-18) | **Nessuno dei due numeri e sbagliato**, ed e questo che rende il problema strutturale. L'asse deve coprire la traiettoria **cumulata** del P&L; il corpo della candela copre solo l'escursione **dentro un bucket**. Su bucket da un giorno le due grandezze differiscono di ordini di grandezza e nessuna impostazione dell'asse puo riconciliarle. Le 310 piatte sono per giunta **corrette**: il portafoglio non detiene asset prima del 2026-07-30 (prima e solo liquidita e versamenti), e una candela sintetica composta su zero asset ha `{o,h,l} = close`. La correlazione e esatta: le 44 candele con escursione reale cadono tutte dopo il primo BUY. **La prova che il difetto e strutturale e non un artefatto dei dati finti arriva dall'arricchimento stesso**: dopo aver esteso le posizioni da 51 a 338 giorni con acquisti mensili a prezzi di chiusura reali, le candele piatte crollano da 310/354 a 15/354 e la finestra 3M del cruscotto non ne ha piu nessuna — eppure **sulla finestra intera la candela mediana vale ancora l'1,75% della campata resa** (~5px su 300), contro il 4,50% a 3 mesi. Il difetto sopravvive alla correzione dei dati: e questo che lo qualifica come strutturale. Si noti anche **quale** statistica si cita: la candela *piu grande* vale l'8,27% in entrambe le finestre, ma e la statistica lusinghiera — un giorno anomalo non dice nulla di cio che l'utente vede lungo un anno. La **mediana** e la misura che caratterizza il difetto. | **Vietato `yAxis: {scale: true}`** come rimedio: `scale:` oggi non compare in `GrowthChart`, quindi l'asse include lo zero per default, e lo zero e portante — la serie P&L e divisa in meta positiva e negativa con riempimento verde sopra e rosso sotto, piu una serie di riferimento tratteggiata. Togliere lo zero dall'inquadratura fa **mentire il grafico sul segno** del P&L dell'utente: un intervallo tutto positivo verrebbe reso come un unico colore indifferenziato. La correzione vera e la **scala delle larghezze candela**: l'aggregazione OHLC prende `max(high)`/`min(low)` sui sotto-bucket, quindi l'escursione e **monotona non decrescente** nella larghezza del bucket — allargare il bucket ingrandisce il corpo mentre la scala dell'asse resta ferma. Resta aperto, e **deliberatamente non incluso** nella scala, lo stato "nessun asset detenuto in questo intervallo": oggi un utente il cui portafoglio inizia a meta finestra vede una linea piatta senza spiegazione. E uno stato vuoto, non un problema di aggregazione, e accorparlo lo nasconderebbe. |
| DBT-8 | **Domanda aperta, non difetto accertato.** Nella finestra 1W il P&L totale fa un giro di andata e ritorno di circa 1300 in un giorno solo: chiusure a ~1641, ~373, ~1984 in tre giorni consecutivi (2026-09-13 il minimo). Non ho prove che sia sbagliato, solo che e sorprendente — puo essere un artefatto dei dati mock della corsia (prezzo mancante, fallback, regolamento) oppure un difetto reale di calcolo del P&L. Va spiegato, non corretto a scatola chiusa. | I, misurazioni su item 2 (2026-09-18) | **Rilevante per il sintomo, non per la discrepanza di I.** I lo aveva classificato ortogonale alla questione rendering: lo e rispetto al suo scarto 3x fra altezza attesa e osservata, perche la sua attesa usava gia la campata reale. **Non lo e rispetto al sintomo del developer**: quel picco cade *dentro* la finestra 1W, e toglierlo porterebbe la campata della serie P&L da ~1611 a ~343 in quell'intervallo. E quindi un contributore di prim'ordine all'altezza dell'asse contro cui le candele vengono schiacciate. Due domande distinte ed entrambe vive: *perche l'osservato sta 3x sotto l'atteso* e *perche l'atteso stesso vale solo l'11%*. | Non assegnato: nessuno deve inseguirlo mentre item 2 e aperto. Quando lo si affronta, la prima discriminante e se il salto esiste anche nei dati grezzi (`price_history` dei singoli asset) o solo nell'aggregato di portafoglio — il secondo caso punta al calcolo, il primo ai dati della corsia. |

**Classe di difetto da sorvegliare, non singolo bug — "eredita il meccanismo, non la semantica".** `get_acquisition_funding_history` era stato modellato su `get_pnl_candles`/`get_broker_pnl_history` e ne aveva ereditato il confine di finestra `>= date_from`, corretto per una serie **livello** (valore cumulato: il primo giorno e una baseline legittima) e sbagliato per una serie **flusso** (un movimento sul primo giorno appartiene al periodo precedente). Corretto in `portfolio_service.py` il 2026-09-18, con la divergenza resa **eseguibile**: un test alimenta lo stesso risultato engine a una serie livello e a una flusso e pretende che dissentano sul giorno di apertura, cosi una futura "armonizzazione" fa diventare rossa esattamente un'asserzione e dichiara quale contratto ha rotto.

Regola che ne discende: quando una nuova serie riusa il *meccanismo* di una esistente, va classificata esplicitamente livello o flusso **prima** di copiarne il confine di finestra. Condividere il motore non implica condividere la semantica temporale.

**La distinzione non e stata inventata per l'occasione: il codice la conosceva gia.** Il ri-basamento ROI di `get_history` usa `s.date <= date_from` per scegliere la baseline NAV (livello: prendi l'ultimo snapshot precedente al periodo) e `cf.date > period_start_date` per i flussi di cassa, con tanto di commento che spiega perche i flussi del giorno iniziale sono gia incorporati nel NAV di apertura. Il modello era corretto nello strato performance e semplicemente non era mai stato propagato allo strato di affettamento delle serie. Questa, piu del fix, e la parte utile: la regola esisteva, mancava la trasmissione. Passata completa 2026-09-18 su tutte e dieci le serie co-renderizzate del pacchetto P&L — ogni serie livello ha inizio inclusivo, ogni serie flusso ha inizio esclusivo, nessun'altra discordanza.

### 11.5 Regole operative emerse in esecuzione

Regole nate da difetti reali di questo round, non da principi astratti. Ognuna cita l'incidente che l'ha generata, perche una regola senza il suo incidente viene rimossa dal primo che la trova scomoda.

**R1 — Quando un contratto wire ha un validator, il validator e la specifica; l'unione di tipi e solo il suo alfabeto.** Incidente: la tabella esposizioni PAC (DBT-3). Coordinatore e implementatore hanno discusso per due turni su quale forma fosse *giusta* — numero pieno o "non disponibile" — argomentando entrambi dal vocabolario dei motivi. Entrambe le risposte erano **inemettibili**: `_validate_weight_availability` ammette due soli mondi e rifiuta tutto il resto. Il tipo dice cosa e *scrivibile*, il validator cosa e *ammesso*: leggere solo il primo produce risposte ben argomentate e sbagliate, che e la categoria piu pericolosa. **Recidiva a poche ore di distanza, vedi DBT-4**: chi ha scritto questa regola l'ha infranta alle 18:39 dello stesso giorno, sullo stesso file, verificando `min_length=1` su un campo e fermandosi prima del validator che governa la sua unione. Non e una lacuna che si chiude una volta: e un cedimento di attenzione che sopravvive al sapere di esistere, e va quindi trattato come un controllo da eseguire, non come un principio da ricordare.

**R2 — Quando la regola di un validator e condizionata da una scelta a monte tua, rendila incondizionata invece di aggiungere una fixture che esercita il ramo cattivo.** Incidente: `provenance_ids` deve essere contenuto nella lista provenance pubblicata; se il report pubblicasse solo le provenance "usate", la violazione comparirebbe **solo** sugli scenari con asset non categorizzati — una suite verde non proverebbe nulla. Risolto pubblicando l'intera lista dello scenario: la regola diventa vera per costruzione. Le fixture decadono, una regola che non puo essere violata no.

**R3 — Uno stub che solleva non puo attraversare un confine di turno.** Incidente: un `raise NotImplementedError` temporaneo durante la costruzione di `planner_report.py`, rimosso nello stesso turno e dichiarato spontaneamente. Se il turno fosse finito male sarebbe sopravvissuto, e `ruff`, `black` e `py_compile` lo approvano tutti senza riserve: sarebbe stato il quinto difetto invisibile ai gate statici in sei stage, famiglia dell'import sotto `TYPE_CHECKING` e dell'attributo inesistente nel percorso di `raise`.

**R4 — Un turno che produce un risultato rilevante per il coordinatore finisce con un messaggio al coordinatore.** Incidente: un turno concluso con un riepilogo all'utente e nessun `send_session_message`. Il danno e concreto, non formale: il coordinatore ricostruisce lo stato dei figli **dall'esterno** — mtime dei file, `py_compile`, `git diff` — e quella vista non vede il lavoro *concluso*. Risultato: una direttiva di resume stantia, che ha richiesto al figlio mezzo turno per correggere. Se un turno sta per chiudersi senza messaggio, e quello stesso il segnale che qualcosa non ha funzionato.

**R5 — Le prove vivono nel repository, non in `/tmp`.** Incidente: 5a e arrivato al confine con 9590 asserzioni verdi, tutte in harness temporanee. Non sono una rete di regressione: sono uno smoke test travestito, a un `rm` dalla scomparsa, e l'unica cosa che separava il modulo da una regressione silenziosa. Regola: una verifica che non sopravvive alla sessione non conta come evidenza in un checkpoint, e i test durevoli si scrivono **prima** del consumatore del modulo, non dopo — altrimenti il primo rosso del percorso combinato e ambiguo fra un difetto del produttore e uno del consumatore.

**R6 — Una mutazione sopravvissuta e prova di una lacuna nei test solo se la mutazione cambia davvero il comportamento.** Incidente: passata di mutazione indipendente su 5a, 3 rilevate e 3 sopravvissute. Nessuna delle tre era una lacuna: una provabilmente irraggiungibile (in forma ridotta `d = 2^a x 5^b` e `n` coprimo con 10, quindi lo scalato non e mai divisibile per 10), una identita (`scaled < 0` equivale a `numerator < 0` perche il fattore e positivo), una mutazione rotta che modificava un commento invece del valore. Segnalarle come lacune avrebbe prodotto test per codice **che non puo eseguire**: verdi per sempre, che non provano nulla e che fissano un contratto mai progettato. Corollario operativo: **verifica che la tua mutazione sia una mutazione vera prima di trarre conclusioni dalla sua sopravvivenza**, perche una mutazione rotta e indistinguibile da una difesa robusta al livello di "la suite e diventata rossa?". E dimostra l'irraggiungibilita per argomento *prima* di confermarla empiricamente: il solo controllo empirico e compatibile con "nessun campione ci e passato".

**R7 — Un ramo irraggiungibile va documentato, non rimosso ne coperto.** Corollario di R6, stessa decisione gia presa per `frozen_exact` in `oracle.py`. Un ramo irraggiungibile *non documentato* e instabile: comparira in ogni report di copertura finche qualcuno non agisce, e le due azioni disponibili sono entrambe cattive. Rimuoverlo fa sparire una difesa reale sulla forza di un numero di copertura. Coprirlo obbliga a costruire un input che il tipo vieta, cioe a scrivere un test che **asserisce il comportamento per un input impossibile**, fissando un contratto che nessuno ha progettato e facendo sembrare una regressione il futuro irrigidimento dell'invariante. Il commento deve contenere **l'argomento di irraggiungibilita**, non la sola parola: se l'invariante a monte cambia, il ramo torna vivo e quel commento e l'unico avviso che qualcosa e cambiato sotto.

**R8 — Un test che ri-deriva l'invariante e un test; un test che osserva l'assenza dell'assert di produzione e un'eco.** Tecnica emersa dalla mutazione A di test-author sulla chiusura delle esposizioni: disabilitare la guardia di produzione e pretendere che il rosso arrivi comunque, perche nasce dal ricalcolo indipendente dell'identita nel test. E la contromisura esatta alla forma "passa silenziosamente perche non ha mai valutato nulla".

**R9 — Un dettaglio interno di una libreria non e un pericolo finche non se ne dimostra la raggiungibilita attraverso il confine pubblico.** Nata da un mio errore, non da uno di un figlio. Diagnosticando il crash delle candele avevo letto in `whiskerBoxCommon.js` che ECharts fa `item.unshift(index)` sui dati ricevuti, e ne avevo dedotto una trappola: il riuso di un array di quad tra due `setOption` avrebbe prepeso l'ordinale due volte, spostando il quad in silenzio. Il codice sorgente diceva esattamente questo. **Ma la trappola non e raggiungibile**: ECharts clona l'opzione in ingresso *prima* che quel codice giri, quindi la mutazione colpisce il clone interno, mai l'array del chiamante. I ha verificato in tre modi, incluso il `setOption` parziale con la forma esatta (`{notMerge:false, replaceMerge:['dataZoom']}`) del percorso di merge reale, e l'array torna intatto ogni volta. E la forma **speculare** di R1: li si legge un vincolo e ci si ferma prima del validator che lo governa; qui si legge un interno e si ignora il confine che lo governa. In entrambi i casi la lettura e corretta e la conclusione e sbagliata, perche il pezzo letto non era quello che decide. Un pericolo dedotto ma non riprodotto va dichiarato **come ipotesi da verificare**, mai consegnato come difetto.

**R10 — Verificando l'affermazione di un figlio si legge il worktree del figlio: il branch di destinazione non e una versione piu vecchia, e un universo diverso.** Nata da un mio errore, scoperto da I che me lo ha contestato con le prove. Avevo dichiarato che `aggregateSumSeries` era completamente priva di test — 414 righe, 25 casi, 0 occorrenze — e ci avevo costruito sopra una decisione, allargando lo scope del refactor condiviso. Il numero era reale e il file era sbagliato: avevo letto **`dev_release2`**, il checkout principale, dove quel file e davvero a 0; nel worktree di I e a 538 righe, 35 casi, 23 occorrenze, perche test-author aveva chiuso quel buco nel batch 2. `git branch --contains d5e834de4` risponde `e-alfy-performance-charts-plan` **e basta**. I aveva diagnosticato "hai letto il commit padre": e una spiegazione piu benigna di quella vera. Non e un errore di recency recuperabile stando attenti alle date: **il coordinatore segue cinque worktree e il target e per costruzione indietro rispetto a tutte**, quindi ogni file non ancora integrato letto nel checkout principale mostra lo stato pre-workstream — in silenzio, senza errore, e sempre **sbilanciato nella stessa direzione**: fa sembrare non fatto il lavoro consegnato. Lasciato correre, produce una sottovalutazione sistematica di ogni figlio, ed e invisibile perche leggere il branch sbagliato restituisce un file perfettamente valido. La verifica indipendente resta il mestiere del coordinatore, ma va fatta **dove il lavoro vive**.

**Corollario di R10, emerso nello stesso scambio.** La correzione di un figlio va verificata come la sua affermazione. Nel confutare il mio numero, I ha corretto anche la mappa del modulo condiviso e ha sbagliato: sosteneva che `groupPointsByBucket` avesse esattamente tre consumatori e che `aggregateEnvelope` raggruppasse per conto suo. I siti sono **quattro**: `:134`, `:158`, `:181` e `:239`, quest'ultimo dentro `aggregateEnvelope`, che non chiama mai `mapDateToBucket`. Conseguenza concreta: `aggregateEnvelope` e **dentro** il raggio del refactor, non fuori "per costruzione". I aveva comunque proposto di catturarne i golden, ma per la ragione sbagliata ("raggruppa in modo indipendente, semmai lo unifico in futuro"); la ragione vera e immediata e che **e gia unificato e si sta cambiando cio che chiama**. Un'azione giusta poggiata su un modello sbagliato e a un refactor di distanza dall'essere eliminata da chi legge "indipendente" e toglie la rete a una funzione che sta nel percorso. **Il meccanismo dell'errore vale piu dell'errore**, e I lo ha ricostruito da solo: il suo pattern di grep catturava sia le dichiarazioni di funzione sia le chiamate a `mapDateToBucket`, quindi nell'output `:230 export function aggregateEnvelope` compariva subito **sopra** `:278` e `:293`. Ha letto il contenimento dalla **vicinanza nell'output del proprio grep** — un artefatto del pattern, non una proprieta del codice. Il guard e meccanico e riusabile: quando si attribuisce una chiamata a una funzione che la contiene, si **risolve la funzione contenitrice** (`awk` sul range, o una lettura del blocco) invece di dedurla dalla prossimita fra le righe stampate. E la quarta istanza della stessa famiglia dopo R1, R9 e R10: dato reale, letto correttamente, conclusione tratta su una relazione che l'output non codificava. Conseguenza operativa adottata: **"ho verificato X" e una frase incompleta finche non dice che cosa e stato misurato**, non soltanto che cosa se ne e concluso.

**R11 — Si misura la proprieta che si sta affermando, non una che le assomiglia; e l'artefatto che si ha gia in mano si apre prima di ragionare su cosa avrebbe mostrato.** Nata da un errore di I, riconosciuto e riportato da lui senza che glielo chiedessi. Per provare che le candele si disegnavano aveva contato i pixel verdi e rossi sulla canvas e aveva riportato "181 verdi + 1331 rossi" come prova. I pixel erano reali e il conteggio corretto, ma misurava **presenza di colore**, non **visibilita di una forma**: non poteva distinguere "i corpi delle candele sono disegnati" da "esiste qualcosa di colorato da qualche parte", che era l'unica cosa che contava. Rimisurando per colonne invece che per somma: 102 colonne colorate su 93 punti, ma la corsa verticale piu alta dell'intera canvas e **11px su 360**. Le candele c'erano sempre state, erano solo invisibili. Il dettaglio piu tagliente e un altro: nella stessa sessione aveva catturato **anche** lo screenshot della submode candele e non l'aveva mai aperto — aveva guardato quello della modalita linea, visto che era sano, e cancellati entrambi. L'artefatto decisivo era gia nelle sue mani. E la terza istanza in una settimana della stessa famiglia di R1, R9 e R10, due delle quali mie: la lettura e competente, l'oggetto e **di un passo a lato** di quello che decide. Un proxy vale come prova solo se codifica la cosa affermata, e quando l'evidenza diretta e gia disponibile, dedurre e una scelta, non una necessita.

**Polarita (addendum R11, 2026-09-18).** Due controlli miei hanno fallito nello stesso giorno, entrambi da una riga, entrambi riportando su se stessi invece che sull'oggetto — ma con conseguenze asimmetriche, distinzione che devo a I. Il `grep -c` sulla tabella dei debiti ha fallito **chiuso**: contava righe corrispondenti invece di occorrenze, su una riga sola, e ha risposto "nessuna duplicazione" mentre la riga era corrotta a sette celle; costo, un minuto di ricontrollo. Lo script i18n ha fallito **aperto**: il filtro di path assumeva `i18n/en/*.json` contro `i18n/en.json` reali, la ricerca del file tornava vuota, e lo script ha rappresentato quel nulla come `MISSING` su tutte e quattro le lingue — cioe ha **fabbricato un difetto** su chiavi sane, e consegnato avrebbe mandato qualcuno a riparare cio che non era rotto. L'asimmetria governa come si scrive il controllo, non solo come si legge il risultato: quando una ricerca puo tornare vuota, il caso vuoto va reso distinguibile da una risposta negativa **prima** che il risultato venga riportato. E il controllo umile non e esente perche e umile.

**R12 — Quando un terzo scrive nello stesso albero, si dichiara cosa includere, mai cosa escludere.** Nata da un errore di I, intercettato prima che facesse danno. Consegnando il checkpoint UI aveva avvertito di non mettere in staging `__tests__/fixtures/`, perche test-author stava scrivendo i golden in quel momento: avvertimento corretto, processo mutante correttamente identificato, e **comunque insufficiente**. I path non tracciati erano due, non uno — `timeSeriesAggregationGolden.test.ts` sta fuori da quella cartella ed e stato creato mentre lui scriveva il messaggio. Chi avesse seguito l'istruzione alla lettera con `git add -A` meno `fixtures/` avrebbe portato dentro il golden spec, esattamente l'esito che l'avvertimento esisteva per impedire. Il meccanismo: **una lista di esclusione e illimitata**, deve nominare ogni membro di un insieme che sta ancora crescendo ed e stantia nell'istante in cui viene scritta; **una lista di inclusione e chiusa**, nomina cio che esiste e nessun terzo puo aggiungervi elementi. Non e un errore di oggetto (R9/R10) ne di costrutto (R11): e un'affermazione **corretta e completa** su un insieme la cui appartenenza e cambiata fra la scrittura e la lettura — il fratello temporale della regola che I aveva derivato un'ora prima per le proprie modifiche (*dichiarare lo stato al momento dell'invio, o non dichiararlo*), senza vedere che si generalizza alle modifiche altrui. **Corollario simmetrico**, emerso subito dopo: il blocco per conflitto si delimita **ai path** che il terzo sta scrivendo, non all'albero. I si era fermato del tutto mentre i file che gli servivano (`GrowthChart.svelte`) erano disgiunti da quelli di test-author. Stessa forma da lati opposti: li l'insieme era troppo stretto per escluderne, qui troppo largo per bloccarsi.

**R13 — Il sintomo affiora nello strato che lo mostra, che quasi mai e lo strato che lo ha causato: prima di formulare ipotesi su come si comporta uno strato, si misura cio che gli e stato passato.** Nata da un difetto di I, trovato da I dopo che erano morte tutte e tre le ipotesi in campo, le due mie e la sua. Sintomo: in submode candele, **zero** pixel del colore della serie nell'area del grafico, mentre la serie esiste e la legenda ne disegna il quadratino. Entrambi abbiamo generato ipotesi **dentro ECharts** — io due (l'asse rimasto `type: 'time'` sul percorso di aggiornamento parziale, che non nomina mai `type`; e il disallineamento fra `entry.dates` e le date su cui la serie era stata costruita), lui una (la dipendenza dallo zoom) — **perche il sintomo era fatto di pixel**. Ed ECharts era l'unico componente della pila senza nulla di cui rispondere: gli erano stati passati 93 sentinella `'-'` e ha disegnato fedelmente 93 nulla, piu il quadratino della serie che comunque esisteva. La causa stava due strati sopra: `resolutionCache` (`:233`) e una `Map` **chiavata sulla sola risoluzione** mentre il valore dipende da sette ingressi, e viene svuotata solo da `resetResolutionState()` (`:421`), invocata solo sotto `if (history !== lastHistoryRef)` (`:1203`). `pnlCandles` arriva **dopo**, per costruzione: e il fetch pigro. Il primo render popola la cache mentre la mappa e ancora vuota, il fetch atterra, l'`$effect` ri-renderizza davvero perche `pnlCandles` e fra le sue dipendenze — e `getResolutionData()` restituisce la voce calcolata **prima che il dato esistesse**. L'artefatto e piu tagliente della descrizione: nello stesso scorcio di schermo l'`$effect` dichiara **sette** dipendenze (`pctSeries`, `eurLabels`, `pnlLabels`, `brokerPnlHistory`, `pnlCandles`, `incomeHistory`, `$locale`) e subito sotto invalida la cache su **una** sola. **La lista che sveglia e la lista che invalida sono adiacenti e discordanti.** Il numero che ha fatto collassare la ricerca e uno solo: `candleNonGap: 0`. Un intero ha convertito una domanda aperta sul rendering in una domanda chiusa sugli ingressi, e andava chiesto per primo — al posto di un'ora di teoria su una libreria che stava funzionando. Conseguenza strutturale adottata: l'insieme che invalida un memo si **deriva da cio che il calcolo memoizzato legge**, non dalla lista delle prop e non dalla lista delle dipendenze dell'effect; una lista mantenuta a mano e un difetto in attesa della prossima prop, e invalidare su tutta la lista delle dipendenze baratta un bug silenzioso-sbagliato con uno silenzioso-lento. **Coda di R11**: ogni pixel di "candela" misurato in tutta la giornata, fino al primissimo "181 verdi + 1331 rossi", era il **quadratino della legenda** — riquadro 15x9 contro `itemWidth: 14, itemHeight: 8` — e il difetto e sopravvissuto a tre strumenti di misura perche nessuno dei tre sapeva distinguere una candela da cio che le stava accanto. Da registrare anche il modo in cui le ipotesi sono morte: **entrambi abbiamo ritirato la nostra su prove nostre prima che l'altro la confutasse**, io sulla forma dei dati (il commento a `:1011` diceva che le quaterne erano corrette), lui sulla dipendenza dallo zoom, con quattro letture identiche al pixel che potevano solo imbarazzare chi le aveva raccolte.
...
| T0/T2 hub e PAC | Round 7: scope, funding, Broker, Asset/route, target, FX, strategy, review, risultati desktop/mobile | Sidebar Tool, scenari manuali/copiati, calcolo, stati invalid/infeasible/busy/stale e lettura delle azioni Broker. |
