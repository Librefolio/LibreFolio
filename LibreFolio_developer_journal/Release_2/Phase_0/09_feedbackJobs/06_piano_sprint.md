...
| DBT-7 | Nella submode candele del P&L il corpo della candela e quasi sempre **invisibile** su finestre ampie: sui dati di review 310 candele su 354 sono piatte (`high == low`), la mediana dell'escursione e `0.0000`, la massima e `249.69` — contro una scala Y che copre `4391.27`, cioe la candela piu grande dell'intero dataset vale il **5,7% dell'altezza dell'asse**. A ~12 unita per pixel anche la candela piu grande e alta ~20px e la mediana e letteralmente zero. A schermo si vede una linea sottile. Il rapporto percentuale e il numero da citare: sopravvive a qualunque cambio di dataset, mentre i valori assoluti no. | I, re-review UI (2026-09-18) | **Nessuno dei due numeri e sbagliato**, ed e questo che rende il problema strutturale. L'asse deve coprire la traiettoria **cumulata** del P&L; il corpo della candela copre solo l'escursione **dentro un bucket**. Su bucket da un giorno le due grandezze differiscono di ordini di grandezza e nessuna impostazione dell'asse puo riconciliarle. Le 310 piatte sono per giunta **corrette**: il portafoglio non detiene asset prima del 2026-07-30 (prima e solo liquidita e versamenti), e una candela sintetica composta su zero asset ha `{o,h,l} = close`. La correlazione e esatta: le 44 candele con escursione reale cadono tutte dopo il primo BUY. | **Vietato `yAxis: {scale: true}`** come rimedio: `scale:` oggi non compare in `GrowthChart`, quindi l'asse include lo zero per default, e lo zero e portante — la serie P&L e divisa in meta positiva e negativa con riempimento verde sopra e rosso sotto, piu una serie di riferimento tratteggiata. Togliere lo zero dall'inquadratura fa **mentire il grafico sul segno** del P&L dell'utente: un intervallo tutto positivo verrebbe reso come un unico colore indifferenziato. La correzione vera e la **scala delle larghezze candela**: l'aggregazione OHLC prende `max(high)`/`min(low)` sui sotto-bucket, quindi l'escursione e **monotona non decrescente** nella larghezza del bucket — allargare il bucket ingrandisce il corpo mentre la scala dell'asse resta ferma. Resta aperto, e **deliberatamente non incluso** nella scala, lo stato "nessun asset detenuto in questo intervallo": oggi un utente il cui portafoglio inizia a meta finestra vede una linea piatta senza spiegazione. E uno stato vuoto, non un problema di aggregazione, e accorparlo lo nasconderebbe. |
