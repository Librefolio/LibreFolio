...
class FAAssetEventPointOut(FAAssetEventPoint):
    ...
    E.8 — FX conversion fields, populated iff ``target_currency`` was requested
    on the query and the conversion succeeded. If conversion failed (FX rate
    missing), ``value`` stays in the event's native currency and all
    ``original_*``/``fx_*`` fields stay ``None`` — the frontend uses this as a
    signal to hide the event marker from the chart (see §E.8.2 of closure plan).
    """
    ...
    original_value: Optional[Currency] = Field(
        None,
        description="Value before FX conversion. Populated iff target_currency conversion applied (E.8).",
    )
    fx_rate_date: Optional[date_type] = Field(
        None,
        description="Effective date of the FX rate used for conversion (E.8).",
    )
    fx_days_back: Optional[int] = Field(
        None,
        description="Days between event.date and the FX rate date (backward-fill magnitude, E.8).",
    )
