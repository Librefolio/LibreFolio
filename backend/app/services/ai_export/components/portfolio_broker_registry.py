...
if len(PORTFOLIO_BROKER_COMPONENTS) != PORTFOLIO_REAL_COMPONENT_COUNT + BROKER_REAL_COMPONENT_COUNT:
    raise PortfolioBrokerRegistryError(f"PORTFOLIO_BROKER_COMPONENTS must contain exactly {PORTFOLIO_REAL_COMPONENT_COUNT + BROKER_REAL_COMPONENT_COUNT} real component_ids, got {len(PORTFOLIO_BROKER_COMPONENTS)}")
if len(PORTFOLIO_REAL_COMPONENT_IDS) != PORTFOLIO_REAL_COMPONENT_COUNT:
    raise PortfolioBrokerRegistryError(f"expected {PORTFOLIO_REAL_COMPONENT_COUNT} Portfolio component_ids, got {len(PORTFOLIO_REAL_COMPONENT_IDS)}")
if len(BROKER_REAL_COMPONENT_IDS) != BROKER_REAL_COMPONENT_COUNT:
    raise PortfolioBrokerRegistryError(f"expected {BROKER_REAL_COMPONENT_COUNT} Broker component_ids, got {len(BROKER_REAL_COMPONENT_IDS)}")
