    ...
    {#snippet resultRow(pr: SyncResult, syncing: boolean)}
        ...
        <div class="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300 group">
            ...
            {#if pr.status === 'ok' || pr.status === 'partial'}
                ...
                {#if pr.provider_used}
                    <span class="inline-flex items-center px-1 py-0.5 text-[9px] font-medium rounded {PROVIDER_COLORS[pr.provider_used] ?? DEFAULT_PROVIDER_COLOR}" title={pr.provider_used}>
                        {pr.provider_used}
                    </span>
                {/if}
            {/if}
