...
<script lang="ts">
    ...
    let {label, value, subLabel, changeValue, changePercent, positive, loading = false}: Props = $props();
...
<div class="relative bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 flex flex-col gap-1 overflow-hidden" data-testid="kpi-card">
    ...
    {:else}
        <!-- Primary value -->
        <p class="text-2xl font-bold text-gray-800 dark:text-gray-100 truncate" data-testid="kpi-value">
            {value}
        </p>

        <!-- Change badge (gain / loss) -->
        {#if changeValue !== undefined || changePercent !== undefined}
            <p class="text-sm font-medium {isPositive ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}" data-testid="kpi-change">
        ...
        {#if subLabel}
            <p class="text-xs text-gray-400 dark:text-gray-500 truncate mt-0.5" data-testid="kpi-sublabel">
                {subLabel}
            </p>
        {/if}
    {/if}
