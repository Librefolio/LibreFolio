...
<script lang="ts">
    ...
    import {
        ...
        getAssetProviderIconUrl,
        getFxProviderIconUrl,
        parseProviderChain,
        PROVIDER_COLORS,
    ...
    $effect(() => { if (open) { ensureAssetProvidersCached(); getCurrencyGraph(); } });
    ...
    async function doFxSync(targetIds: string[]): Promise<SyncResult[]> {
        const response = await zodiosApi.sync_rates_api_v1_fx_currencies_sync_post(
            {pairs: targetIds, start: dateStart, end: dateEnd},
            {timeout: 120_000},
        );
        const r = response as any;
...
<SyncModalBase
        ...
        {dateEnd}
        {dateStart}
        description={$t('assetDetail.pageSyncDescription') ?? 'Synchronize asset prices and FX rates for this page.'}
        {onclose}
        {onsynced}
        {sections}
        testId="page-sync-modal"
