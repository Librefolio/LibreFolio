<script lang="ts">
        ...
        if (browser) {
            ...
            try {
                ...
                if (!isAuth) {
                    ...
                    if (appBootstrap.ready && $page.route.id !== '/(app)/welcome' && appBootstrap.resolveDestination(currentPath) === currentPath) {
    ...
    $: if (browser && $isAuthenticated && appBootstrap.ready && !isWelcomeRoute) {
        if (!onboardingRouteReady) {
            void goto(onboardingDestination, {replaceState: true});
        }
    }
    $: if (browser && $isAuthenticated && appBootstrap.ready && onboardingRouteReady && !isWelcomeRoute) {
        onboardingGuide.maybeStartIntro();
    }
