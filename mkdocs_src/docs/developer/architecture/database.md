...
> 💡 **Schema Refactoring (Nov 2025)**: Major code quality improvements completed. See `FEATURE_COVERAGE_REPORT.md` for details.
...
The fields of each object is documented in detail in the [Interest Calculation Guide](../../features/scheduled-investments.md).
...
[//]: # (TODO: creare la Interest Calculation Guide)
The fields are documented in detail in the [Interest Calculation Guide](../../features/scheduled-investments.md).
...
See also:
- [Metadata Management API Examples](../../api/examples.md)
- [Metadata Management Guide](../features/asset-pricing.md)
...
## 🕵️ How to get information about the database schema

To get information about the database schema an Agent can:

1.  Read this file.
2.  Inspect the file `backend/app/db/models.py` to see the SQLModel definitions for all tables.
3.  Inspect the directory `backend/alembic/versions/` to see the migration history. The file `001_initial.py` contains the squashed schema.
4.  Run the database validation tests to check for consistency: `./test_runner.py db validate`
5.  Inspect the live database directly using `sqlite3 backend/data/sqlite/app.db .schema`.
