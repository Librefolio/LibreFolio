# ![](../../../static/icons/asset-types/commodity.png){: width="32" style="vertical-align: middle;" } Materias Primas

Las **Materias Primas** son activos físicos —materias primas o bienes primarios— mantenidos para la preservación del valor a largo plazo o como cobertura contra la inflación. En LibreFolio, esta categoría cubre metales preciosos, energía y otras reservas tangibles de valor.

---

## 🔑 Características Clave

| Propiedad | Detalle |
|----------|--------|
| **Código en LibreFolio** | `HOLD` |
| **Precios** | Precios spot de bolsas de materias primas, o valoración manual |
| **Moneda** | Generalmente cotizadas en USD (oro, petróleo), pero pueden expresarse en cualquier moneda |
| **Dividendos** | Ninguno — las materias primas no generan ingresos |
| **Almacenamiento** | Las materias primas físicas pueden incurrir en costes de almacenamiento/seguro |
| **Proveedores típicos** | Yahoo Finance (ej. `GC=F` para futuros de oro), Manual |

---

## 📊 Materias Primas Comunes

| Materia Prima | Símbolo | Rol en la Cartera |
|-----------|--------|------------------|
| **Oro** | `GC=F` | Refugio seguro, cobertura contra la inflación |
| **Plata** | `SI=F` | Metal industrial + precioso |
| **Petróleo (Crudo)** | `CL=F` | Exposición energética |
| **Gas Natural** | `NG=F` | Exposición energética |

---

## 📈 ¿Por qué invertir en materias primas?

- **Cobertura contra la inflación**: Los precios de las materias primas tienden a subir con la inflación
- **Diversificación**: Baja correlación con acciones y bonos
- **Refugio seguro**: El oro históricamente se comporta bien durante las crisis de mercado
- **Sin riesgo de contraparte**: El oro físico no tiene un emisor que pueda incurrir en impago

---

## 🔗 Relacionado

- 🔀 **[Diversificación](../../portfolio-theory/diversification.md)** — Las materias primas como instrumentos de diversificación de la cartera
- 📊 **[Volatilidad](../../technical-analysis/risk-metrics/volatility.md)** — Medición del riesgo de precio de las materias primas
- 📈 **[Rentabilidades y Tasas de Crecimiento](../../fundamentals/returns.md)** — Medición del rendimiento de las materias primas
