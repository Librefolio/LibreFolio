# ![](../../../static/icons/asset-types/commodity.png){: width="32" style="vertical-align: middle;" } Matières premières

Les **matières premières** sont des actifs physiques — ressources brutes ou biens primaires — détenus pour la préservation de la valeur à long terme ou comme couverture contre l'inflation. Dans LibreFolio, cette catégorie couvre les métaux précieux, l'énergie et d'autres réserves de valeur tangibles.

---

## 🔑 Caractéristiques clés

| Propriété | Détail |
|----------|--------|
| **Code dans LibreFolio** | `HOLD` |
| **Prix** | Prix spot des bourses de matières premières, ou évaluation manuelle |
| **Devise** | Généralement cotées en USD (or, pétrole) mais peuvent être libellées dans n'importe quelle devise |
| **Dividendes** | Aucun — les matières premières ne génèrent pas de revenus |
| **Stockage** | Les matières premières physiques peuvent engendrer des coûts de stockage/assurance |
| **Fournisseurs typiques** | Yahoo Finance (ex: `GC=F` pour les contrats à terme sur l'or), Manuel |

---

## 📊 Matières premières courantes

| Matière première | Symbole | Rôle dans le portefeuille |
|-----------|--------|------------------|
| **Or** | `GC=F` | Valeur refuge, couverture contre l'inflation |
| **Argent** | `SI=F` | Métal industriel + précieux |
| **Pétrole brut** | `CL=F` | Exposition à l'énergie |
| **Gaz Naturel** | `NG=F` | Exposition à l'énergie |

---

## 📈 Pourquoi détenir des matières premières

- **Couverture contre l'inflation** : Les prix des matières premières ont tendance à augmenter avec l'inflation
- **Diversification** : Faible corrélation avec les actions et les obligations
- **Valeur refuge** : L'or s'est historiquement bien comporté en période de crise
- **Aucun risque de contrepartie** : L'or physique n'a pas d'émetteur pouvant faire défaut

---

## 🔗 Liens connexes

- 🔀 **[Diversification](../../portfolio-theory/diversification.md)** — Les matières premières comme diversificateurs de portefeuille
- 📊 **[Volatilité](../../technical-analysis/risk-metrics/volatility.md)** — Mesure du risque de prix des matières premières
- 📈 **[Rendements & Taux de croissance](../../fundamentals/returns.md)** — Mesure de la performance des matières premières
