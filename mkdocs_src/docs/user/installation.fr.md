...
Ce guide explique comment déployer LibreFolio pour un usage régulier à l'aide de Docker. C'est la méthode recommandée pour les utilisateurs qui n'ont pas l'intention de modifier le code source.
...
!!! note "Pourquoi Python et Node.js ?"
    ...
    LibreFolio utilise une **image Docker limitée à l'exécution (runtime)** — le frontend et la documentation sont construits sur l'hôte avant d'être empaquetés dans l'image Docker. Des images pré-construites sur un registre de conteneurs sont prévues pour les futures versions.
...
1. **Copiez le fichier d'exemple** (requis — la construction refusera de continuer sans `.env`) :
...
2. **Modifiez `.env`** pour personnaliser :
    ...
    - 💰 `PORTFOLIO_BASE_CURRENCY` : Devise de base du portefeuille (par défaut : `EUR`).
...
3. L'empaquetage de l'ensemble dans une seule image Docker avec le tag `librefolio:latest`
...
- 🔄 `-d` lance l'application en mode détaché (en arrière-plan).
...
!!! tip "Gestion des utilisateurs via CLI"
    ...
    Vous pouvez également gérer les utilisateurs depuis la ligne de commande. Consultez le [Manuel Admin — Outils CLI](../admin/cli_tools.md) pour les commandes de création, de promotion et de listage d'utilisateurs.
...
2. **Reconstruisez l'image Docker** (reconstruit automatiquement le frontend et la doc s'ils ont été modifiés) :

    ```bash
    ./dev.py docker rebuild
    ```

    Cette commande construit une nouvelle image, arrête les conteneurs en cours d'exécution et redémarre avec la nouvelle version.
...
Vous pouvez lancer un serveur de test avec des données de simulation pré-remplies pour explorer l'application avant de saisir vos données réelles :
...
Accès à **`http://localhost:6041`** avec l'utilisateur `e2e_test_user` / `E2eTestPass123!`.
